...
def main():
    ...
    for i, part_name in enumerate(all_parts):
        aligned_json = os.path.join(
            WORKSPACE, part_name.replace(".m4a", "_aligned.json")
        )
        ...
        for w in part_words:
            ...
            if i > 0 and word_mid <= (offset + OVERLAP / 2.0):
                keep = False
            if i < len(all_parts) - 1 and word_mid > (
                metadata[all_parts[i + 1]]["start_offset"] + OVERLAP / 2.0
            ):
                keep = False
            if keep:
    ...
    for i, w in enumerate(global_words):
        ...
        if w.get("is_sentence_end", False):
            ...
            split_condition = True
        elif i < len(global_words) - 1 and (
            global_words[i + 1]["abs_start"] - w["abs_end"] > SILENCE_GAP
        ):
            split_condition = True
        ...
        if split_condition or i == len(global_words) - 1:
            if sub_segment:
                ...
                final_segments.append(
                    {"start": start_time, "end": end_time, "text": text_content.strip()}
                )
            sub_segment = []
